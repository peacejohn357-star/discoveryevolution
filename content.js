...
(function () {
  ...
  function handleTick(tick) {
    ...
    if (cfg.strategyMode === 'discoveryEvolution' && dnaWorker) {
      ...
      const { currentRSI, currentBBW, currentStr } = currentMetrics;
      let tickAction = "IDLE";
      ...
      if (lastMetrics) {
        ...
        } else if (!armedDiscoverySignal && rollingDirections.length === 5 && rollingDirections !== lastProcessedSeq && patternLibrary[rollingDirections]) {
            // 5. Arming check — only when not armed AND sequence is newly formed (streak lock)
            const match = patternLibrary[rollingDirections].find(p =>
                p.regime === currentRegime &&
                Math.abs(currentRSI - p.indicators.rsi) < 0.3 &&
                Math.abs(currentBBW - p.indicators.bbw) < 0.02 &&
                Math.abs(currentStr - p.indicators.str) < 0.05
            );
            if (match) {
                lastProcessedSeq = rollingDirections; // Lock: ignore this seq until it changes
                armedDiscoverySignal = {
                    action: match.action,
                    sequence: rollingDirections,
                    hitState: match.indicators,  // needed by fail-path renderer & logDiscoveryFailure
                    regime: match.regime || 'D'
                };
                // Increment count on the library entry so duplicates are always tracked
                match.count++;
                // Register in activeTradePool so Armed counter reflects reality
                ...
                console.log("[EVO] ARMED:", rollingDirections, match.action, `(count: ${match.count})`);
            }
        }
      } else {
      ...
      discoveryAuditLog.push({
        ...
        bbw: currentBBW.toFixed(4),
        strain: currentStr.toFixed(2),
        status: tickAction,
    ...
    try {
      ...
      dnaWorker.onmessage = function(e) {
        if (e.data.type === 'signal') {
          ...
          if (!patternLibrary[sequence]) patternLibrary[sequence] = [];

          // Tight dedup: RSI, BBW, STR must all be near-identical AND regime must match exactly.
          // A different regime = a different market context = a new distinct pattern entry.
          let match = patternLibrary[sequence].find(p =>
            p.regime === regime &&
            Math.abs(p.indicators.rsi - hitState.rsi) < 0.3 &&
            Math.abs(p.indicators.bbw - hitState.bbw) < 0.02 &&
  ...
  function updateDiscoveryUI(s, isFail = false, actualMetrics = null) {
    ...
    // Display lookup uses same tight tolerances as dedup so count is always accurate
    const libEntry = patternLibrary[s.sequence]
        ? patternLibrary[s.sequence].find(p =>
            p.regime === s.regime &&
            Math.abs(p.indicators.rsi - s.hitState.rsi) < 0.3 &&
            Math.abs(p.indicators.bbw - s.hitState.bbw) < 0.02 &&
            Math.abs(p.indicators.str - s.hitState.str) < 0.05)
        : null;
    ...
    // Find existing row for this sequence and update in-place, or create a new one
    let entry = feed.querySelector(`[data-seq="${CSS.escape(s.sequence)}"]`);
    if (entry) {
        entry.innerHTML = rowHTML;
    } else {
        entry = document.createElement('div');
        entry.setAttribute('data-seq', s.sequence);
    ...
    // Update total unique pattern count in header
    const patCountEl = document.getElementById('tt-pattern-count');
    if (patCountEl) patCountEl.textContent = `[Patterns: ${feed.childNodes.length}]`;
  }
